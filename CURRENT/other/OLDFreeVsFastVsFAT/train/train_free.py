...
def do_free_train(train_loader,args,model,delta,criterion,opt,epsilon,scheduler):
    train_loss = 0
    train_acc = 0
    train_n = 0
    for i, (X, y) in enumerate(train_loader):
            ...
            train_n += y.size(0)
    return train_loss, train_acc, train_n, scheduler
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        train_loss, train_acc, train_n, scheduler = do_free_train(train_loader,args,model,delta,criterion,opt,epsilon,scheduler)
        # for i, (X, y) in enumerate(train_loader):
        #     X, y = X.cuda(), y.cuda()
        #     for _ in range(args.minibatch_replays):
        #         output = model(X + delta[:X.size(0)])
